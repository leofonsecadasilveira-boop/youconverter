...
export default function CompressTool({ isDark }) {
  ...
  return (
    <div>
      <h2 style={{fontSize:22,fontWeight:900,marginBottom:16,color:titleColor}}>Comprimir PDF</h2>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12}}>
        {Object.entries(MODES).map(([k,m])=>(
          <button key={k} onClick={()=>setMode(k)} style={getBtn(mode===k)}>
            <div style={{fontWeight:900,fontSize:12,color:mode===k?PURPLE:(isDark?'#f3f4f6':'#111827'), lineHeight:'1.2'}}>{m.label}</div>
            <div style={{fontSize:10,color:mode===k?PURPLE:(isDark?'#9ca3af':'#6b7280'), marginTop:4, fontWeight:600}}>{m.badge}</div>
          </button>
        ))}
      </div>
      <div style={{background: isDark?'rgba(91,33,182,0.12)':'rgba(91,33,182,0.08)', border:`1px solid ${PURPLE}`, borderRadius:8, padding:'8px 12px', fontSize:11, color:isDark?'#c4b5fd':PURPLE, marginBottom:16, textAlign:'center', fontWeight:600}}>
        {MODES[mode].desc}
      </div>
      ...
      <button onClick={handle} disabled={loading||!file} style={{marginTop:16,background:PURPLE,color:'#fff',border:0,padding:'16px 26px',borderRadius:10,fontWeight:900,cursor:'pointer',width:'100%',opacity:loading||!file?0.35:1}}>
