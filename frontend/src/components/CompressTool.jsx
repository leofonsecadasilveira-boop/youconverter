...
export default function CompressTool({ isDark }) {
  ...
  return (
    <div>
      ...
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:12}}>
        {Object.entries(MODES).map(([k,m])=>{
          ...
          <button key={k} onClick={()=>setMode(k)} style={{
            ...
            <div style={{fontWeight:800,fontSize:12,color:active?'#ffffff':(isDark?'#f3f4f6':'#111827'), lineHeight:'1.2'}}>{m.label}</div>
            <div style={{fontSize:10,color:active?'#ffffff':(isDark?'#9ca3af':'#6b7280'), marginTop:4, fontWeight:600, opacity: active?0.9:1}}>{m.badge}</div>
          </button>
        )})}
      </div>
      <div style={{background: isDark?'rgba(124,58,237,0.18)':'rgba(124,58,237,0.08)', border:`1px solid ${PURPLE}`, borderRadius:8, padding:'8px 12px', fontSize:11, color:isDark?'#e9d5ff':PURPLE, marginBottom:16, textAlign:'center', fontWeight:600}}>
        {MODES[mode].desc}
      </div>
      ...
      {stats && <div style={{marginTop:12, background:isDark?'#052e16':'#ECFDF5', border:'1px solid #A7F3D0', padding:'12px', borderRadius:8, fontSize:13, color:isDark?'#6ee7b7':'#065F46', fontWeight:700, textAlign:'center'}}>✅ {stats.from} MB → {stats.to} MB {stats.pct>0?`(-${stats.pct}%)`:''}</div>}
      <button onClick={handle} disabled={loading||!file} style={{marginTop:16,background:PURPLE,color:'#fff',border:0,padding:'14px 24px',borderRadius:10,fontWeight:800,cursor:'pointer',width:'100%',opacity:loading||!file?0.6:1}}>
