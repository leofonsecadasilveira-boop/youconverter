...
export default function UnlockTool({ isDark }) {
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      const a = document.createElement('a'); a.href = url; a.download = file.name.replace(/\.pdf$/i,'') + '-desbloqueado.pdf'
      document.body.appendChild(a); a.click(); a.remove()
      setTimeout(() => URL.revokeObjectURL(url), 1000)
    } catch (e) { alert('Senha incorreta ou PDF não protegido: ' + e.message) } finally { setLoading(false) }
  }
  const inputStyle = { marginTop:12, width:'100%', padding:'12px', borderRadius:8, border:isDark?'1px solid #3f3f46':'1px solid #e5e7eb', background:isDark?'#18181b':'white', color:isDark?'#fff':'#111827', boxSizing:'border-box' }
  return (
    <div>
      ...
      <button onClick={handleUnlock} disabled={loading || !file} style={{ marginTop:16, background:PURPLE, color:'#fff', border:0, padding:'14px 24px', borderRadius:10, fontWeight:800, cursor:'pointer', width:'100%', opacity:loading||!file?0.6:1 }}>
