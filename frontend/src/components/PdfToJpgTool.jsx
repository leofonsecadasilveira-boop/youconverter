...
export default function PdfToJpgTool({ isDark }) {
  ...
  return (
    <div>
      <h2 style={{fontSize:22,fontWeight:800, boxShadow:'0 4px 14px rgba(124,58,237,0.35)',marginBottom:16,color:titleColor}}>PDF para JPG</h2>
      <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:12}}>
        <button onClick={()=>setQuality('alta')} style={getBtn(quality==='alta')}>
          <div style={{fontWeight:800, boxShadow:'0 4px 14px rgba(124,58,237,0.35)',fontSize:12,color:quality==='alta'?PURPLE:(isDark?'#f3f4f6':'#111827')}}>Alta qualidade</div>
        ...
        <button onClick={()=>setQuality('media')} style={getBtn(quality==='media')}>
          <div style={{fontWeight:800, boxShadow:'0 4px 14px rgba(124,58,237,0.35)',fontSize:12,color:quality==='media'?PURPLE:(isDark?'#f3f4f6':'#111827')}}>Arquivo menor</div>
      ...
      <div style={{background:bannerBg, border:`1px solid ${PURPLE}`, borderRadius:8, padding:'10px 12px', fontSize:11, color:bannerColor, marginBottom:16, textAlign:'center', fontWeight:600}}>
        🔒 YouConverter Engine • Cada página vira 1 JPG • 100% no seu PC
      </div>
      ...
      <button onClick={handleConvert} disabled={loading||!file} style={{marginTop:16,background:PURPLE,color:'#fff',border:0,padding:'14px 24px',borderRadius:12,fontWeight:800, boxShadow:'0 4px 14px rgba(124,58,237,0.35)',cursor:'pointer',width:'100%',opacity:loading||!file?0.6:1}}>
