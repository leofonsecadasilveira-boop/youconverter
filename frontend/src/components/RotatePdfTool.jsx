      ...
      <div style={{marginTop:16, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8}}>
        {[90,180,270].map(a => (
          <button key={a} onClick={()=>setAngle(a)} style={{
            padding:'12px', borderRadius:10, border: angle===a?'2px solid #5B21B6':'1px solid #2A2A2A',
            background: angle===a?'#2a1f4d':'#1A1A1A', cursor:'pointer', fontWeight:800, fontSize:13,
            color: angle===a?'#5B21B6':'#E5E7EB'
          }}>{a}°</button>
        ))}
      </div>
      <button onClick={handleRotate} disabled={loading || !file} style={{ marginTop: 16, background: '#5B21B6', color: '#fff', border: 0, padding: '16px 26px', borderRadius: 10, fontWeight: 800, cursor: 'pointer', width: '100%', opacity: loading || !file ? 0.35 : 1 }}>
